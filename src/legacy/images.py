...
from .render_metadata import prepare_render_note
...
from ..neural_rendering.image.decoder import decode_image as _decode_image
from ..neural_rendering.image.encoder import _encode_image, take_image_preview
...
def decode_image(source):
    ...
    if decoded.alpha is None:
        decoded = replace(decoded, alpha=np.full(decoded.rgba.shape[:2], 255, dtype=np.uint8))
    return decoded
