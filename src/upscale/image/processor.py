...
def _process(source, options, progress, output_dir, controller, generate_previews, capabilities, on_image):
    ...
    try:
        ...
        if not save_output:
            update(.98, "Returning image without saving")
            if on_image is not None:
                with Image.fromarray(processed) as rendered:
                    on_image(rendered, source.name)
            update(1.0, "Complete; not saved")
            return ImageUpscaleResult(str(source), "", "", ow, oh,
                                      time.monotonic() - started, list(decoded.warnings))
